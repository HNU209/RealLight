...
class RealLight:
    def __init__(self, args):
        ...
        self.actor = Actor(self.args).to(device=self.device)
        self.critic = Critic(self.args).to(device=self.device)
    ...
    def get_action(self, state):
        state = torch.from_numpy(np.array(state, dtype=np.float32)).to(device=self.device)
        prob = self.actor(state)
        prob = prob.detach().numpy()
        action = np.random.choice(self.args['out_dim'], 1, p=prob)[0]
    ...
    def make_batch(self):
        ...
        s_batch = torch.from_numpy(np.array(s_lst, dtype=np.float32)).to(device=self.device)
        a_batch = torch.from_numpy(np.array(a_lst, dtype=np.int64)).to(device=self.device)
        prob_batch = torch.from_numpy(np.array(prob_lst, np.float32)).to(device=self.device)
        r_batch = torch.from_numpy(np.array(r_lst, np.float32)).to(device=self.device)
        s_prime_batch = torch.from_numpy(np.array(s_prime_lst, np.float32)).to(device=self.device)
        done_batch = torch.from_numpy(np.array(done_lst, np.float32)).to(device=self.device)
    ...
    def train(self):
        ...
        for _ in range(self.k_epoch):
            ...
            for i in range(n // self.batch_size):
                ...
                prob_ = self.actor(s_samples, softmax_dim=1)
                curr_v_ = self.critic(s_samples)
                
                log_prob = torch.log(prob_ + 1e-8)
                action_prob = prob_.gather(1, a_samples)
                action_log_prob = torch.log(action_prob + 1e-8)
                action_log_old_prob = torch.log(prob_old_samples + 1e-8)
                
                ratio = (action_log_prob - action_log_old_prob).sum(1, keepdim=True)
                ...
                entropy_loss = -torch.sum(prob_ * log_prob, dim=-1).mean()
